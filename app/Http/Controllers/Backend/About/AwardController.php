...
class AwardController extends Controller
{
	public function __construct()
	{
		...
		$this->resource_view = 'backend.about.award.';
	}
	...
	{
		if($id==false){
			$model = $this->model->whereParentId(null)->select('id' , 'title', 'status')->whereCategory('award');
		}else{

			$model = $this->model->whereParentId($id)->select('id' , 'title', 'parent_id', 'image', 'created_at', 'status')->whereCategory('award');
		}
		...
		return Table::of($model)
			->addColumn('images' , function($model){
				if(!empty($model->image)){
					...
					$images ='<img src="'.asset('contents/award/thumbnail').'/'.$model->image.'" width="200" height="200" />';
                }else{
	...
	public function postCreate(Request $request,$id = false)
	{
			...
			if($id==false){
		        ...
		        if(!empty($image))
		        {
					$imageName = "award-".$save->id;
					$uploadImage = \Helper::handleUpload($request, $imageName, 'award');
					// dd($uploadImage);
