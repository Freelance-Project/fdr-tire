...
class ProfileController extends Controller
{
	public function __construct()
	{
		...
		$this->category = 1;
		$this->resource_view = 'backend.about.profile.';
	}
	...
	public function getData()
	{
		...
		$data = $this->model->whereParentId(null)->where('category','aboutus')->first();

		$model = $this->model->whereParentId($data->id)->select('id' , 'title', 'brief', 'description','created_at', 'status')->whereCategory('aboutus');
	...
	{	
		$model = $this->model->whereParentId(null)->where('category','aboutus')->first();
	...
	public function postIndex(Request $request)
	{
        ...
        if(!empty($image))
        {
			$imageName = "aboutus-".$dataid;
			$uploadImage = \Helper::handleUpload($request, $imageName, 'aboutus');
			// dd($uploadImage);
